...
while(1):
    ...
    if (frameCount > 1):
        if (mogCount > movementCount):
            print(movementText)
            cv2.putText(mogMask, 'Someones stealing your honey', stealingTextPosition,
                        cv2.FONT_HERSHEY_SIMPLEX, .5, textColor, 2, cv2.LINE_AA)
        if (mog2MCount > movementCount):
            print(movementText)
            cv2.putText(mog2Mmask, 'Someones stealing your honey', stealingTextPosition,
                        cv2.FONT_HERSHEY_SIMPLEX, .5, textColor, 2, cv2.LINE_AA)
        if (gmgCount > movementCount):
            print(movementText)
            cv2.putText(gmgMask, 'Someones stealing your honey', stealingTextPosition,
                        cv2.FONT_HERSHEY_SIMPLEX, .5, textColor, 2, cv2.LINE_AA)
        if (knnCount > movementCount):
            print(movementText)
            cv2.putText(knnMask, 'Someones stealing your honey', stealingTextPosition,
                        cv2.FONT_HERSHEY_SIMPLEX, .5, textColor, 2, cv2.LINE_AA)
        if (cntCount > movementCount):
            print(movementText)
            cv2.putText(cntMask, 'Someones stealing your honey', stealingTextPosition,
                        cv2.FONT_HERSHEY_SIMPLEX, .5, textColor, 2, cv2.LINE_AA)
